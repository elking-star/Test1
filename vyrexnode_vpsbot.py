...
WATERMARK = "VyrexNode VPS Service"
WELCOME_MESSAGE = "Welcome To VyrexNode! Get Started With Us!"
...
class VyrexNodeBot(commands.Bot):
...
async def setup_container(container_id, status_msg, memory, username, vps_id=None, use_custom_image=False):
    """Enhanced container setup with VyrexNode customization"""
    try:
        ...
        if not use_custom_image:
            ...
            packages = [
                ...
                "openssh-server", "sudo", "git", "docker.io", "systemd", "systemd-sysv"
            ]
        ...
        # Set VyrexNode customization
        if isinstance(status_msg, discord.Interaction):
            await status_msg.followup.send("🎨 Setting up VyrexNode customization...", ephemeral=True)
        else:
            await status_msg.edit(content="🎨 Setting up VyrexNode customization...")
        ...
        hostname_cmd = f"echo 'vyrexnode-{vps_id}' > /etc/hostname && hostname vyrexnode-{vps_id}"
        ...
        # Basic security setup (ufw doesn't work in containers, use iptables instead)
        security_commands = [
        ...
        if isinstance(status_msg, discord.Interaction):
            await status_msg.followup.send("✅ VyrexNode VPS setup completed successfully!", ephemeral=True)
        else:
            await status_msg.edit(content="✅ VyrexNode VPS setup completed successfully!")
...
bot = VyrexNodeBot(command_prefix='/', intents=intents, help_command=None)
...
async def on_ready():
    ...
    try:
        await bot.change_presence(activity=discord.Activity(type=discord.ActivityType.watching, name="VyrexNode VPS"))
...
async def show_commands(ctx):
    ...
    try:
        embed = discord.Embed(title="🤖 VyrexNode VPS Bot Commands", color=discord.Color.blue())
...
@app_commands.describe(
    ...
    use_custom_image="Use custom VyrexNode image (recommended)"
)
async def create_vps_command(ctx, memory: int, cpu: int, disk: int, owner: discord.Member, 
    ...
    try:
        ...
        status_msg = await ctx.send("🚀 Creating VyrexNode VPS instance... This may take a few minutes.")
        ...
        if use_custom_image:
            ...
            try:
                container = bot.docker_client.containers.run(
                    ...
                    volumes={
                        f'vyrexnode-{vps_id}': {'bind': '/data', 'mode': 'rw'}
                    },
        ...
        await status_msg.edit(content="🔧 Container created. Setting up VyrexNode environment...")
        await asyncio.sleep(5)
        ...
        try:
            embed = discord.Embed(title="🎉 VyrexNode VPS Creation Successful", color=discord.Color.green())
            ...
            embed.add_field(name="ℹ️ Note", value="This is a VyrexNode VPS instance. You can install and configure additional packages as needed.", inline=False)
            
            await owner.send(embed=embed)
            await status_msg.edit(content=f"✅ VyrexNode VPS creation successful! VPS has been created for {owner.mention}. Check your DMs for connection details.")
...
async def list_vps(ctx):
    ...
    try:
        ...
        embed = discord.Embed(title="Your VyrexNode VPS Instances", color=discord.Color.blue())
...
async def admin_list_vps(ctx):
    ...
    try:
        ...
        embed = discord.Embed(title="All VyrexNode VPS Instances", color=discord.Color.blue())
        valid_vps_count = 0
...
async def delete_vps(ctx, vps_id: str):
    ...
    try:
        ...
        await ctx.send(f"✅ VyrexNode VPS {vps_id} has been deleted successfully!")
    except Exception as e:
...
async def connect_vps(ctx, token: str):
    ...
    try:
        ...
        embed = discord.Embed(title="VyrexNode VPS Connection Details", color=discord.Color.blue())
...
4. You will be connected to your VyrexNode VPS
...
""".format(username=vps["username"]), inline=False)
        ...
        await ctx.send("✅ Connection details sent to your DMs! Use the Tmate command to connect to your VyrexNode VPS.", ephemeral=True)
...
async def admin_stats(ctx):
    ...
    try:
        ...
        embed = discord.Embed(title="VyrexNode System Statistics", color=discord.Color.blue())
...
async def vps_usage(ctx):
    ...
    try:
        ...
        embed = discord.Embed(title="Your VyrexNode VPS Usage", color=discord.Color.blue())
...
async def global_stats(ctx):
    ...
    try:
        ...
        embed = discord.Embed(title="VyrexNode Global Usage Statistics", color=discord.Color.blue())
...
async def reinstall_bot(ctx):
    ...
    try:
        await ctx.send("🔄 Reinstalling VyrexNode bot... This may take a few minutes.")
...
"""
        ...
        process = await asyncio.create_subprocess_exec(
            "docker", "build", "-t", "vyrexnode-bot", "-f", "Dockerfile.bot", ".",
            stdout=asyncio.subprocess.PIPE,
            stderr=asyncio.subprocess.PIPE
        )
...
class VPSManagementView(ui.View):
    ...
    async def handle_missing_container(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(title=f"VyrexNode VPS Management - {self.vps_id}", color=discord.Color.red())
    ...
    async def start_vps(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            embed = discord.Embed(title=f"VyrexNode VPS Management - {self.vps_id}", color=discord.Color.green())
            ...
            await interaction.followup.send("✅ VyrexNode VPS started successfully!", ephemeral=True)
        except Exception as e:
    ...
    async def stop_vps(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            embed = discord.Emembed(title=f"VyrexNode VPS Management - {self.vps_id}", color=discord.Color.orange())
            ...
            await interaction.message.edit(embed=embed)
            await interaction.followup.send("✅ VyrexNode VPS stopped successfully!", ephemeral=True)
        except Exception as e:
    ...
    async def restart_vps(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            if token:
                ...
                try:
                    ...
                    if ssh_session_line:
                        ...
                        try:
                            owner = await bot.fetch_user(int(vps["created_by"]))
                            embed = discord.Embed(title=f"VyrexNode VPS Restarted - {self.vps_id}", color=discord.Color.blue())
            ...
            await interaction.followup.send("✅ VyrexNode VPS restarted successfully! New SSH details sent to owner.", ephemeral=True)
        except Exception as e:
...
class OSSelectionView(ui.View):
    ...
    async def reinstall_os(self, interaction: discord.Interaction, image: str):
        try:
            ...
            status_msg = await interaction.followup.send("🔄 Reinstalling VyrexNode VPS... This may take a few minutes.", ephemeral=True)
            ...
            try:
                container = bot.docker_client.containers.run(
                    ...
                    detach=True,
                    privileged=True,
                    hostname=f"vyrexnode-{self.vps_id}",
                    mem_limit=memory_bytes,
                    cpu_period=100000,
                    ...
                    network=DOCKER_NETWORK,
                    volumes={
                        f'vyrexnode-{self.vps_id}': {'bind': '/data', 'mode': 'rw'}
                    }
                )
            except docker.errors.ImageNotFound:
            ...
            try:
                ...
                if ssh_session_line:
                    ...
                    try:
                        owner = await bot.fetch_user(int(vps["created_by"]))
                        embed = discord.Embed(title=f"VyrexNode VPS Reinstalled - {self.vps_id}", color=discord.Color.blue())
            ...
            await status_msg.edit(content="✅ VyrexNode VPS reinstalled successfully!")
            ...
            try:
                ...
                try:
                    ...
                    await channel.send(f"❌ Error reinstalling VyrexNode VPS {self.vps_id}: {str(e)}")
                except:
...
class TransferVPSModal(ui.Modal, title='Transfer VPS'):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            await interaction.response.send_message(f"✅ VyrexNode VPS {self.vps_id} has been transferred from {old_owner_name} to {new_owner_name}!", ephemeral=True)
            
            try:
                embed = discord.Embed(title="VyrexNode VPS Transferred to You", color=discord.Color.green())
...
async def manage_vps(ctx, vps_id: str):
    ...
    try:
        ...
        embed = discord.Embed(title=f"VyrexNode VPS Management - {vps_id}", color=discord.Color.blue())
...
async def transfer_vps_command(ctx, vps_id: str, new_owner: discord.Member):
    ...
    try:
        ...
        await ctx.send(f"✅ VyrexNode VPS {vps_id} has been transferred from {ctx.author.name} to {new_owner.name}!")
